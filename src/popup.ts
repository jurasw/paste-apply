...
function populateFormFields(data: ProfileData): void {
...
async function parseResume(): Promise<void> {
...
function injectFillFunction(profileData: ProfileData, resumeData: ResumeData): void {
  ...
  function findFieldByKeywords(keywords: string[], value: string): boolean {
    ...
    for (const input of allInputs) {
      const htmlInput = input as HTMLInputElement;
      if (htmlInput.type === 'hidden' || htmlInput.type === 'submit' || htmlInput.type === 'button') continue;
      if (htmlInput.value && htmlInput.value.trim() !== '') continue;
    ...
    for (const label of labels) {
      ...
      for (const keyword of labelKeywords) {
        if (labelText.includes(keyword) && !labelText.includes('country')) {
          ...
          const parentInput = label.parentElement?.querySelector('input, textarea') as HTMLInputElement;
