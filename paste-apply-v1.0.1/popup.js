...
function uploadResumeFile(resumeData) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        fillBtn.addEventListener('click', async () => {
            try {
                ...
                const profileData = await loadProfileData();
                const resumeData = await loadResumeData();
                const hasProfileData = profileFields.some(field => profileData[field] && profileData[field].trim() !== '');
                const hasResumeData = resumeData.resumeFile;
                if (!hasProfileData && !hasResumeData) {
                    showStatus('Profile is empty. Please fill in your profile first.', 'error');
                    return;
                }
                try {
